...
Query.prototype.bind = function bind (model, op, updateArg) {
  ...
  this._debug = !! model.prototype.base.options.debug;
  return this;
};
...
Query.prototype.execFind = function (callback) {
  ...
  if (this._debug) this._print('find', castQuery, options);
...
Query.prototype.each = function (callback) {
  ...
  if (this._debug) this._print('find', this._conditions, options);
...
Query.prototype.findOne = function (callback) {
  ...
  if (this._debug) this._print('findOne', castQuery, options);
...
Query.prototype.count = function (callback) {
  ...
  if (this._debug) this._print('count', castQuery, {});
...
Query.prototype.distinct = function (field, callback) {
  ...
  if (this._debug) this._print('distinct', castQuery, {});
...
Query.prototype.update = function update (doc, callback) {
  ...
  if (castDoc) {
    if (this._debug) this._print('update', castQuery, castDoc, options);
    model.collection.update(castQuery, castDoc, options, tick(callback));
  } else {
    process.nextTick(function () {
      if (this._debug) this._print('update', 'no-op');
      callback(null);
    });
  }
...
Query.prototype.remove = function (callback) {
  ...
  if (this._debug) this._print('remove', castQuery, options);
...
/**
 * Print information about the query being sent.
 * @private
 */

Query.prototype._print = function (method, query, up, opt) {
  console.error('\x1B[0;35mMongoose:\x1B[0m %s.%s\n %s\n %s\n %s'
